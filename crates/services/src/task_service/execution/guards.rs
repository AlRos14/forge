...
impl TaskService {
    ...
    pub(super) async fn busy_only_because_current_execution(
        ...
        let canonical_role =
            db::canonical_task_role_name(&execution.role).unwrap_or_else(|| execution.role.clone());
        let role_authorized =
            match current_role_memberships_authoritative(&self.db, &task.id, &canonical_role)
                .await?
            {
